...
BUILD_STAMP = os.environ.get("BRIDGENA_BUILD", "v3.9.0-openwebui-control-redesign")
...
def page(title: str, content: str, active: str = "", raw_js: str = "", wide: bool = False) -> str:
    nav=[('dash','/dashboard','Overview'),('chat','/chat','Chat'),('browser','/browser-view','Browsers'),('pool','/pool','Network'),('jars','/jars','Accounts'),('models','/models-page','Models'),('keys','/api-keys','API keys'),('errors','/errors','Errors'),('logs','/logs','Logs')]
    icons={'dash':'⌂','chat':'✦','browser':'▣','pool':'⌁','jars':'◎','models':'◇','keys':'⌘','errors':'!','logs':'≡'}
    links=''.join(f'<a class="{"on" if active==k else ""}" href="{h}"><span style="width:17px;text-align:center;font-size:13px">{icons.get(k,"·")}</span><span>{esc(l)}</span></a>' for k,h,l in nav)
    body_class=' wide' if wide else ''
    return f'''<!doctype html><html data-theme="dark"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="dark light"><title>{esc(title)} · Bridgena</title><style>{CSS}</style></head><body>
<header class="topbar"><a class="brand" href="/dashboard"><span class="dot"></span><span>Bridgena</span></a><span style="margin-left:9px;color:var(--ink3);font-size:11px">Control Plane</span><div style="margin-left:auto;display:flex;align-items:center;gap:7px"><a class="btn ghost" href="/chat">Open chat</a><button class="btn ghost" onclick="bgnTheme()" title="Theme">◐</button><a class="btn ghost" href="/logout">Sign out</a></div></header>
<div class="shell"><aside class="rail">{links}</aside><main class="main{body_class}">{content}</main></div><div id="bgn-toasts" class="toast-stack"></div>
<script>function bgnTheme(){{const r=document.documentElement,n=r.dataset.theme==='light'?'dark':'light';r.dataset.theme=n;localStorage.setItem('bgn.theme',n)}}document.documentElement.dataset.theme=localStorage.getItem('bgn.theme')||'dark';function bgnToast(message,type='ok'){{const root=document.getElementById('bgn-toasts'),el=document.createElement('div');el.className='toast '+type;el.textContent=message;root.appendChild(el);setTimeout(()=>el.remove(),3000);return el}}function bgnToastUpdate(el,message,type='ok'){{if(!el)return;el.textContent=message;el.className='toast '+type}}{raw_js}</script></body></html>'''

def login_page(err: str = "") -> str:
    return f'''<!doctype html><html data-theme="dark"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="dark light"><title>Sign in · Bridgena</title><style>{CSS}
.authroot{{min-height:100vh;display:grid;place-items:center;padding:24px;background:radial-gradient(900px 500px at 50% -15%,color-mix(in srgb,var(--ink) 7%,transparent),transparent 70%),var(--bg)}}.authbox{{width:min(410px,100%);animation:authin .42s var(--ease)}}.authlogo{{width:44px;height:44px;border-radius:15px;background:var(--brand);color:var(--brandInk);display:grid;place-items:center;margin:0 auto 22px;font-weight:800}}.authbox h1{{font-size:27px;letter-spacing:-.045em;text-align:center;margin:0 0 7px}}.authbox .sub{{text-align:center;color:var(--ink3);margin-bottom:26px}}.authcard{{padding:22px;border-radius:24px}}.authcard label{{display:block;font-size:12px;color:var(--ink2);margin:0 0 7px}}.authcard input{{width:100%;height:44px;padding:0 13px}}.authcard button{{width:100%;height:44px;margin-top:12px;border:0;border-radius:13px;background:var(--brand);color:var(--brandInk);font-weight:650;cursor:pointer}}.autherr{{padding:10px 12px;margin-bottom:12px;border-radius:12px;background:color-mix(in srgb,var(--bad) 10%,transparent);color:var(--bad);border:1px solid color-mix(in srgb,var(--bad) 24%,transparent)}}.authfoot{{text-align:center;color:var(--ink3);font-size:11px;margin-top:18px}}@keyframes authin{{from{{opacity:0;transform:translateY(8px) scale(.985)}}to{{opacity:1;transform:none}}}}</style></head><body><div class="authroot"><div class="authbox"><div class="authlogo">B</div><h1>Welcome back</h1><div class="sub">Sign in to your Bridgena workspace.</div><form class="authcard" method="post" action="/login">{f'<div class="autherr">{esc(err)}</div>' if err else ''}<label for="password">Admin password</label><input id="password" name="password" type="password" autocomplete="current-password" autofocus placeholder="Enter your password"><button type="submit">Continue</button></form><div class="authfoot">Bridgena control plane · private workspace</div></div></div><script>document.documentElement.dataset.theme=localStorage.getItem('bgn.theme')||'dark'</script></body></html>'''
...
:root{--bg:#0b0b0d;--side:#111114;--soft:#18181c;--soft2:#1e1e23;--line:#29292f;--text:#f5f5f6;--muted:#aaaab3;--dim:#73737d;--ok:#50d890;--bad:#f07182;--warn:#eeb85d;--shadow:0 22px 70px rgba(0,0,0,.34);--ease:cubic-bezier(.2,.8,.2,1)}
[data-theme=light]{--bg:#fff;--side:#f7f7f8;--soft:#f1f1f3;--soft2:#ebebee;--line:#e2e2e6;--text:#171719;--muted:#66666d;--dim:#9898a0;--ok:#15995d;--bad:#dc4960;--warn:#a66d0d;--shadow:0 22px 62px rgba(20,20,25,.10)}
*{box-sizing:border-box}html,body{margin:0;height:100%;overflow:hidden;background:var(--bg);color:var(--text);font:14px/1.6 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;-webkit-font-smoothing:antialiased}
button,input,textarea{font:inherit}.app{height:100%;display:grid;grid-template-columns:258px minmax(0,1fr)}.side{display:flex;flex-direction:column;background:var(--side);border-right:1px solid var(--line);min-width:0}.sidehead{height:58px;display:flex;align-items:center;gap:10px;padding:0 14px}.logo{width:30px;height:30px;border-radius:11px;background:var(--text);color:var(--bg);display:grid;place-items:center;font-size:11px;font-weight:800}.sidebody{flex:1;min-height:0;overflow:auto;padding:7px}
.new,.thread,.ghost,.modelbtn,.chipbtn{border:0;color:inherit;cursor:pointer;transition:.15s var(--ease)}.new{width:100%;height:40px;border-radius:12px;text-align:left;padding:0 12px;background:transparent;font-weight:560}.new:hover,.thread:hover,.ghost:hover,.modelbtn:hover,.chipbtn:hover{background:var(--soft)}.label{padding:22px 11px 8px;color:var(--dim);text-transform:uppercase;font-size:9px;font-weight:700;letter-spacing:.1em}.thread{width:100%;display:flex;background:transparent;border-radius:11px;padding:9px 11px;color:var(--muted);text-align:left}.thread span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.thread.on{background:var(--soft);color:var(--text)}
.sidefoot{padding:9px;border-top:1px solid var(--line)}.sidefoot a{display:block;color:var(--muted);text-decoration:none;padding:10px 11px;border-radius:11px}.sidefoot a:hover{background:var(--soft);color:var(--text)}.main{position:relative;min-width:0;min-height:0;display:flex;flex-direction:column;background:var(--bg)}.top{height:58px;display:flex;align-items:center;gap:8px;padding:0 15px;border-bottom:1px solid color-mix(in srgb,var(--line) 78%,transparent);background:color-mix(in srgb,var(--bg) 88%,transparent);backdrop-filter:blur(18px)}
.modelwrap{position:relative}.modelbtn{height:36px;max-width:min(480px,60vw);border-radius:12px;padding:0 11px;background:transparent;font-weight:620;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.status{margin-left:auto;display:flex;align-items:center;gap:7px;color:var(--muted);font-size:11px}.status i{width:7px;height:7px;border-radius:50%;background:var(--ok)}.ghost{width:36px;height:36px;border-radius:12px;background:transparent}.picker{display:none;position:absolute;top:43px;left:0;z-index:70;width:min(500px,calc(100vw - 28px));background:var(--side);border:1px solid var(--line);border-radius:17px;box-shadow:var(--shadow);overflow:hidden;animation:pop .16s var(--ease)}.picker.open{display:block}.picker input{width:100%;height:44px;border:0;border-bottom:1px solid var(--line);outline:0;background:transparent;color:var(--text);padding:0 13px}.modellist{max-height:380px;overflow:auto;padding:6px}.modelopt{width:100%;border:0;border-radius:10px;background:transparent;color:var(--text);padding:9px 11px;text-align:left;cursor:pointer}.modelopt:hover,.modelopt.on{background:var(--soft)}
.scroll{flex:1;min-height:0;overflow:auto;overscroll-behavior:contain;scrollbar-gutter:stable}.conversation{width:min(860px,100%);margin:0 auto;padding:48px 24px 210px}.welcome{min-height:calc(100vh - 260px);display:grid;place-items:center;text-align:center}.welcome h1{font-size:34px;line-height:1.15;letter-spacing:-.05em;margin:0 0 9px;font-weight:680}.welcome p{margin:0;color:var(--muted)}.suggestions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin-top:26px;width:min(590px,100%)}.chipbtn{padding:13px 14px;text-align:left;border:1px solid var(--line);border-radius:15px;background:var(--side);color:var(--muted)}.chipbtn:hover{transform:translateY(-1px);color:var(--text);box-shadow:0 8px 24px rgba(0,0,0,.10)}
.msg{display:grid;grid-template-columns:34px minmax(0,1fr);gap:14px;margin-bottom:36px;animation:msgin .22s var(--ease)}.avatar{width:32px;height:32px;border-radius:12px;background:var(--soft);display:grid;place-items:center;font-size:10px;font-weight:800}.msg.user{display:flex;justify-content:flex-end;margin-left:20%}.msg.user .avatar,.msg.user .head{display:none}.msg.user .body{max-width:84%;padding:10px 15px;border-radius:18px 18px 5px 18px;background:var(--soft)}.head{font-size:11px;color:var(--dim);margin:3px 0 5px;font-weight:650}.body{font-size:15px;line-height:1.78;word-break:break-word}.body pre{overflow:auto;background:#08080a;border:1px solid var(--line);border-radius:14px;padding:14px;font:12px/1.65 ui-monospace,SFMono-Regular,Menlo,monospace}.body code{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;background:var(--soft);border-radius:6px;padding:2px 5px}.body pre code{background:none;padding:0}.reason{margin:0 0 13px;color:var(--muted);font-size:12px;border-left:2px solid var(--line);padding:7px 0 7px 11px;white-space:pre-wrap}.error{color:var(--bad)}
.dock{position:absolute;left:0;right:0;bottom:0;padding:70px 18px 18px;background:linear-gradient(transparent,var(--bg) 48%);pointer-events:none}.compose{pointer-events:auto;width:min(860px,100%);margin:0 auto;border:1px solid var(--line);border-radius:22px;background:color-mix(in srgb,var(--bg) 94%,transparent);backdrop-filter:blur(18px);padding:12px;box-shadow:0 14px 46px rgba(0,0,0,.20);transition:.18s var(--ease)}.compose:focus-within{border-color:color-mix(in srgb,var(--text) 24%,var(--line));box-shadow:0 16px 52px rgba(0,0,0,.24),0 0 0 3px color-mix(in srgb,var(--text) 5%,transparent)}.compose textarea{display:block;width:100%;min-height:44px;max-height:200px;resize:none;border:0;outline:0;background:transparent;color:var(--text);padding:5px 6px;line-height:1.55}.composefoot{display:flex;align-items:center;color:var(--dim);font-size:10px}.send{margin-left:auto;width:35px;height:35px;border:0;border-radius:12px;background:var(--text);color:var(--bg);cursor:pointer;transition:.15s var(--ease)}.send:hover{transform:scale(1.04)}.send.stop{background:var(--bad);color:#fff}.runtime{position:fixed;z-index:90;top:70px;right:14px;bottom:14px;width:min(520px,calc(100vw - 28px));background:var(--side);border:1px solid var(--line);border-radius:20px;box-shadow:var(--shadow);padding:13px;display:none;animation:drawer .2s var(--ease)}.runtime.open{display:flex;flex-direction:column}.runtime pre{flex:1;overflow:auto;background:var(--bg);border:1px solid var(--line);border-radius:13px;padding:11px;color:var(--muted);white-space:pre-wrap;font:11px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace}
.mobile{display:none}@keyframes pop{from{opacity:0;transform:translateY(-5px) scale(.985)}to{opacity:1;transform:none}}@keyframes drawer{from{opacity:0;transform:translateX(12px)}to{opacity:1;transform:none}}@keyframes msgin{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:none}}
@media(max-width:760px){.app{grid-template-columns:1fr}.side{position:fixed;inset:0 auto 0 0;width:min(286px,86vw);z-index:100;transform:translateX(-105%);transition:.2s var(--ease);box-shadow:var(--shadow)}.side.open{transform:none}.mobile{display:block}.conversation{padding:30px 15px 195px}.suggestions{grid-template-columns:1fr}.msg.user{margin-left:7%}.status span{display:none}.dock{padding-inline:10px}.modelbtn{max-width:58vw}}
@media(prefers-reduced-motion:reduce){*{transition:none!important;animation:none!important;scroll-behavior:auto!important}}
</style></head><body><div class=app><aside class=side id=side><div class=sidehead><div class=logo>B</div><b>Bridgena</b></div><div class=sidebody><button class=new onclick="newChat()">＋ New chat</button><div class=label>Recent</div><div id=threads></div></div><div class=sidefoot><a href="/dashboard">⚙ Admin settings</a></div></aside><main class=main><header class=top><button class="ghost mobile" onclick="side.classList.toggle('open')">☰</button><div class=modelwrap><button class=modelbtn id=modelBtn onclick="togglePicker()">Model</button><div class=picker id=picker><input id=modelSearch placeholder="Search models"><div class=modellist id=modelList></div></div></div><div class=status><i id=statusDot></i><span id=statusText>Ready</span></div><button class=ghost onclick="toggleRuntime()">⌁</button><button class=ghost onclick="toggleTheme()">◐</button></header><div class=scroll id=scroll><div class=conversation id=conversation></div></div><div class=dock><div class=compose><textarea id=input placeholder="Message Bridgena"></textarea><div class=composefoot><span>Enter to send · Shift+Enter newline</span><button class=send id=send onclick="sendOrStop()">↑</button></div></div></div></main></div><aside class=runtime id=runtime><div style="display:flex;align-items:center"><b>Runtime signal</b><button class=ghost style="margin-left:auto" onclick="toggleRuntime()">×</button></div><pre id=signal>Waiting for activity…</pre></aside><script>
const MODELS=__MODELS__,DEFAULT_MODEL=__DEFAULT__,$=id=>document.getElementById(id),side=$('side');let controller=null,busy=false,model=localStorage.getItem('bgn.v3.model')||DEFAULT_MODEL,chatId=localStorage.getItem('bgn.v3.chat')||newId();function newId(){return 'c-'+crypto.getRandomValues(new Uint32Array(2)).join('-')}function store(){try{return JSON.parse(localStorage.getItem('bgn.v3.chats')||'{}')}catch(e){return {}}}function saveStore(v){localStorage.setItem('bgn.v3.chats',JSON.stringify(v))}function current(){const s=store();return s[chatId]||{id:chatId,title:'New chat',messages:[],updated:Date.now()}}function saveCurrent(c){const s=store();s[chatId]=c;saveStore(s);localStorage.setItem('bgn.v3.chat',chatId);renderThreads()}function esc(s){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}function md(s){let x=esc(s);x=x.replace(/```([\s\S]*?)```/g,(_,b)=>'<pre><code>'+b+'</code></pre>');x=x.replace(/`([^`]+)`/g,'<code>$1</code>');x=x.replace(/\*\*([^*]+)\*\*/g,'<strong>$1</strong>');return x.replace(/\n/g,'<br>')}function renderThreads(){const s=store(),items=Object.values(s).sort((a,b)=>(b.updated||0)-(a.updated||0)).slice(0,60);$('threads').innerHTML=items.map(c=>'<button class="thread '+(c.id===chatId?'on':'')+'" onclick="openChat(\''+c.id.replace(/'/g,'')+'\')"><span>'+esc(c.title||'New chat')+'</span></button>').join('')}function openChat(id){chatId=id;localStorage.setItem('bgn.v3.chat',id);renderConversation();renderThreads();side.classList.remove('open')}function newChat(){chatId=newId();saveCurrent({id:chatId,title:'New chat',messages:[],updated:Date.now()});renderConversation()}function renderConversation(){const c=current(),root=$('conversation');root.innerHTML='';if(!c.messages.length){root.innerHTML='<div class=welcome><div><div class=logo style="margin:0 auto 16px;width:42px;height:42px">B</div><h1>How can I help?</h1><p>Ask anything, inspect code, or work through a production issue.</p><div class=suggestions><button class=chipbtn onclick="usePrompt(\'Review this code and identify reliability risks\')">Review code</button><button class=chipbtn onclick="usePrompt(\'Help me diagnose a failed request\')">Diagnose a request</button></div></div></div>';return}c.messages.forEach(m=>appendRendered(m.role,m.content,m.reasoning||''));scrollBottom(false)}function appendRendered(role,content,reasoning=''){const root=$('conversation'),w=root.querySelector('.welcome');if(w)w.remove();const d=document.createElement('div');d.className='msg '+role;d.innerHTML='<div class=avatar>'+(role==='user'?'U':'B')+'</div><div><div class=head>'+(role==='user'?'You':'Bridgena')+'</div><div class=body>'+(reasoning?'<div class=reason>'+esc(reasoning)+'</div>':'')+md(content)+'</div></div>';root.appendChild(d);return d.querySelector('.body')}function setStatus(t,state='ok'){$('statusText').textContent=t;$('statusDot').style.background=state==='bad'?'var(--bad)':state==='busy'?'var(--warn)':'var(--ok)'}function scrollBottom(s=true){$('scroll').scrollTo({top:$('scroll').scrollHeight,behavior:s?'smooth':'auto'})}function toggleTheme(){const r=document.documentElement,n=r.dataset.theme==='light'?'dark':'light';r.dataset.theme=n;localStorage.setItem('bgn.theme',n)}document.documentElement.dataset.theme=localStorage.getItem('bgn.theme')||'dark';function togglePicker(){$('picker').classList.toggle('open');if($('picker').classList.contains('open')){$('modelSearch').value='';renderModels('');$('modelSearch').focus()}}function renderModels(q=''){const x=q.toLowerCase(),arr=MODELS.filter(m=>m.toLowerCase().includes(x)).slice(0,250);$('modelList').innerHTML=arr.map(m=>'<button class="modelopt '+(m===model?'on':'')+'" data-m="'+esc(m)+'">'+esc(m)+'</button>').join('')||'<div style="padding:20px;color:var(--muted)">No matching models</div>';document.querySelectorAll('.modelopt').forEach(b=>b.onclick=()=>{model=b.dataset.m;localStorage.setItem('bgn.v3.model',model);$('modelBtn').textContent=model;$('picker').classList.remove('open')})}$('modelSearch').addEventListener('input',e=>renderModels(e.target.value));$('modelBtn').textContent=model;renderModels('');function usePrompt(s){$('input').value=s;$('input').focus();autoSize()}function autoSize(){const t=$('input');t.style.height='44px';t.style.height=Math.min(190,t.scrollHeight)+'px'}$('input').addEventListener('input',autoSize);$('input').addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendOrStop()}});function toggleRuntime(){$('runtime').classList.toggle('open')}async function refreshSignal(){try{const r=await fetch('/debug-logs/data',{cache:'no-store'});if(!r.ok)return;const d=await r.json();$('signal').textContent=d.slice(-120).map(x=>x.line||x.message||'').join('\n')}catch(e){}}setInterval(refreshSignal,2500);function saveMsg(role,content,reasoning=''){const c=current();c.messages.push({role,content,reasoning,ts:Date.now()});if(c.title==='New chat'&&role==='user')c.title=content.replace(/\s+/g,' ').slice(0,48)||'New chat';c.updated=Date.now();saveCurrent(c)}function sendOrStop(){if(busy){controller?.abort();return}sendMessage()}async function sendMessage(){const input=$('input'),text=input.value.trim();if(!text)return;busy=true;controller=new AbortController();input.value='';autoSize();$('send').textContent='■';$('send').classList.add('stop');setStatus('Generating','busy');saveMsg('user',text);appendRendered('user',text);const body=appendRendered('assistant','');let acc='',reason='';scrollBottom();try{const c=current();const messages=c.messages.slice(-24).map(m=>({role:m.role==='assistant'?'assistant':'user',content:m.content}));const r=await fetch('/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json'},signal:controller.signal,body:JSON.stringify({model,messages,stream:true,chat_id:chatId,stream_options:{include_usage:true}})});if(!r.ok)throw new Error('HTTP '+r.status+': '+await r.text());const rd=r.body.getReader(),dec=new TextDecoder();let buf='';while(true){const {done,value}=await rd.read();if(done)break;buf+=dec.decode(value,{stream:true});let cut;while((cut=buf.indexOf('\n'))>=0){const line=buf.slice(0,cut).trim();buf=buf.slice(cut+1);if(!line.startsWith('data: '))continue;const raw=line.slice(6);if(raw==='[DONE]')continue;let j;try{j=JSON.parse(raw)}catch(e){continue}if(j.error)throw new Error(j.error.message||'Bridge stream error');const d=j.choices?.[0]?.delta||{};if(d.reasoning_content)reason+=d.reasoning_content;if(d.content)acc+=d.content;body.innerHTML=(reason?'<div class=reason>'+esc(reason)+'</div>':'')+md(acc);scrollBottom(false)}}if(!acc)throw new Error('The upstream completed without assistant content.');saveMsg('assistant',acc,reason);setStatus('Ready')}catch(e){if(e.name==='AbortError'){body.innerHTML=(reason?'<div class=reason>'+esc(reason)+'</div>':'')+md(acc||'Generation stopped.');if(acc)saveMsg('assistant',acc,reason);setStatus('Stopped')}else{const err='<div class=error>'+esc(e.message||e)+'</div>';if(acc||reason){body.innerHTML=(reason?'<div class=reason>'+esc(reason)+'</div>':'')+md(acc)+err;if(acc)saveMsg('assistant',acc,reason)}else{body.innerHTML=err}setStatus('Error','bad')}}finally{busy=false;controller=null;$('send').textContent='↑';$('send').classList.remove('stop');renderThreads()}}renderThreads();renderConversation();refreshSignal();document.addEventListener('click',e=>{if(!$('picker').contains(e.target)&&e.target!==$('modelBtn'))$('picker').classList.remove('open')});
</script></body></html>'''
...
async def _lifespan(app):
    ...
    log("INFO", "Control UI · v3.9 settings-pane redesign · squircle/shadcn tokens · responsive chat shell")
