...
class KeeperSession:
    ...
    async def _persist_browser_state(self) -> bool:
        """Persist Playwright storage state without converting a successful
        authenticated browser startup into a fatal keeper failure."""
        try:
            if not self.context:
                return False
            profile_dir = os.path.join(PROFILES_DIR, self.jar_id)
            ...
            await self.context.storage_state(
                path=os.path.join(profile_dir, "state.json")
            )
            return True
        except Exception as exc:
            log(
                "WARN",
                f"[{self.name}] Browser state persistence skipped: "
    ...
    async def _start_impl(self) -> bool:
        ...
        try:
            ...
            _internal_err = isinstance(
                e, (AttributeError, TypeError, NameError, UnboundLocalError)
            )
            _local_route_err = bool(LOCAL_UPSTREAM and
                                    (ARENA_BASE in txt or "localhost:6767" in txt))
            if used:
                if _internal_err:
                    _probe_fail_reason[used] = "keeper internal error (proxy NOT penalized)"
                    log(
                        "WARN",
                        f"[{self.name}] internal keeper error; proxy health unchanged · "
                        f"{txt[:120]}",
                    )
                elif _local_route_err:
                ...
                elif not _local_route_err and not _internal_err:
...
def logs_page(tail: list) -> str:
    import json as _json
    initial = _json.dumps(tail, ensure_ascii=False)
    body = r'''<div class="log-page-head">
  <div><div class="eyebrow">Runtime</div><h1>System log</h1><p>Live keeper, transport and control-plane events.</p></div>
  <div class="log-actions"><button class="btn" id="pauseBtn" onclick="togglePause()">Pause</button><button class="btn danger" onclick="clearLogs()">Clear</button></div>
</div>
<div class="log-toolbar">
  <label class="log-search"><span>Search</span><input id="logSearch" placeholder="Filter log output..." oninput="renderLogs()"></label>
  <div class="log-filters" id="logFilters">
    <button class="log-chip on" data-level="ALL" onclick="setLevel(this)">All</button>
    <button class="log-chip" data-level="INFO" onclick="setLevel(this)">Info</button>
    <button class="log-chip" data-level="OK" onclick="setLevel(this)">OK</button>
    <button class="log-chip" data-level="WARN" onclick="setLevel(this)">Warn</button>
    <button class="log-chip" data-level="ERROR" onclick="setLevel(this)">Error</button>
  </div>
  <div class="log-live"><i></i><span id="liveLabel">Live</span></div>
</div>
<div class="log-shell"><div class="log-console" id="c"></div></div>'''

    css = r'''<style>
.log-page-head{display:flex;align-items:flex-end;gap:20px;margin-bottom:22px}.log-page-head h1{font:600 31px/1.08 var(--serif);letter-spacing:-.03em;margin:3px 0 6px}.log-page-head p{margin:0;color:var(--muted)}.log-actions{margin-left:auto;display:flex;gap:8px}
.log-toolbar{display:flex;align-items:center;gap:10px;margin-bottom:10px;flex-wrap:wrap}.log-search{height:40px;min-width:260px;flex:1;display:flex;align-items:center;gap:8px;border:1px solid var(--line);background:var(--surface);border-radius:16px;corner-shape:squircle;padding:0 12px;color:var(--dim)}.log-search span{font:600 10px var(--sans);text-transform:uppercase;letter-spacing:.06em}.log-search input{height:auto;min-height:0;width:100%;border:0;background:transparent;box-shadow:none;padding:0;color:var(--fg)}
.log-filters{display:flex;gap:5px;padding:4px;border:1px solid var(--line);background:var(--surface);border-radius:15px;corner-shape:squircle}.log-chip{width:auto;height:31px;margin:0;border:0;border-radius:11px;corner-shape:squircle;background:transparent;color:var(--muted);padding:0 10px;font:600 11px var(--sans);cursor:pointer}.log-chip.on{background:var(--surface2);color:var(--fg)}
.log-live{display:flex;align-items:center;gap:7px;color:var(--muted);font-size:11px;padding:0 8px}.log-live i{width:8px;height:8px;border-radius:99px;background:var(--good);box-shadow:0 0 0 5px color-mix(in srgb,var(--good) 11%,transparent)}
.log-shell{border:1px solid var(--line);border-radius:28px;corner-shape:squircle;background:var(--surface);overflow:hidden;box-shadow:0 18px 48px rgba(23,16,10,.16)}.log-console{height:calc(100dvh - 255px);min-height:430px;overflow:auto;background:color-mix(in srgb,var(--surface2) 82%,var(--bg));padding:8px!important;border:0!important;border-radius:0!important;font-family:"SFMono-Regular",Consolas,"Liberation Mono",monospace!important}
.log-row{display:grid;grid-template-columns:76px 64px minmax(0,1fr);gap:10px;align-items:start;min-height:34px;padding:7px 10px;border-radius:12px;corner-shape:squircle;color:var(--muted);font:500 11.5px/1.55 "SFMono-Regular",Consolas,"Liberation Mono",monospace}.log-row:hover{background:color-mix(in srgb,var(--surface3) 76%,transparent)}.log-time{color:var(--dim);font-variant-numeric:tabular-nums}.log-level{justify-self:start;border-radius:999px;padding:2px 7px;font:700 9px/1.45 var(--sans);letter-spacing:.04em}.log-msg{color:color-mix(in srgb,var(--fg) 88%,var(--muted));word-break:break-word}
.lvl-INFO .log-level{background:color-mix(in srgb,#7aa2f7 12%,transparent);color:#9bb8ef}.lvl-OK .log-level{background:color-mix(in srgb,var(--good) 12%,transparent);color:var(--good)}.lvl-WARN .log-level{background:color-mix(in srgb,var(--warn) 13%,transparent);color:var(--warn)}.lvl-ERROR .log-level{background:color-mix(in srgb,var(--bad) 13%,transparent);color:var(--bad)}.log-empty{padding:60px 20px;text-align:center;color:var(--dim);font-family:var(--serif)}
@media(max-width:720px){.log-page-head{align-items:flex-start;flex-direction:column}.log-actions{margin-left:0}.log-row{grid-template-columns:62px 55px minmax(0,1fr)}.log-console{height:calc(100dvh - 300px)}.log-search{min-width:100%}}
</style>'''

    js = r'''
let LOGS=__INITIAL__,paused=false,activeLevel='ALL',stick=true;
function eLog(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function splitLine(x){const raw=String(x.line||x.message||'');const m=raw.match(/^\[([^\]]+)\]\s*\[([^\]]+)\]\s*(.*)$/);return {time:m?m[1]:'',level:String(x.lvl||(m?m[2]:'INFO')).toUpperCase(),msg:m?m[3]:raw}}
function renderLogs(){const c=document.getElementById('c'),q=(document.getElementById('logSearch').value||'').toLowerCase(),near=c.scrollHeight-c.scrollTop-c.clientHeight<100;const rows=LOGS.map(splitLine).filter(x=>(activeLevel==='ALL'||x.level===activeLevel)&&(!q||(x.time+' '+x.level+' '+x.msg).toLowerCase().includes(q)));c.innerHTML=rows.length?rows.map(x=>'<div class="log-row lvl-'+eLog(x.level)+'"><span class="log-time">'+eLog(x.time)+'</span><span class="log-level">'+eLog(x.level)+'</span><span class="log-msg">'+eLog(x.msg)+'</span></div>').join(''):'<div class="log-empty">No matching log entries.</div>';if(stick&&(near||c.dataset.first!=='1')){c.scrollTop=c.scrollHeight;c.dataset.first='1'}}
function setLevel(b){activeLevel=b.dataset.level;document.querySelectorAll('.log-chip').forEach(x=>x.classList.toggle('on',x===b));renderLogs()}
function togglePause(){paused=!paused;document.getElementById('pauseBtn').textContent=paused?'Resume':'Pause';document.getElementById('liveLabel').textContent=paused?'Paused':'Live'}
async function pullLogs(){if(paused)return;try{const r=await fetch('/debug-logs/data',{cache:'no-store'});if(!r.ok)return;LOGS=await r.json();renderLogs()}catch(e){}}
async function clearLogs(){const t=bgnToast('Clearing logs...','loading');try{const r=await fetch('/clear-logs',{method:'POST'});if(!r.ok)throw new Error('Could not clear logs');LOGS=[];renderLogs();bgnToastUpdate(t,'Logs cleared','ok')}catch(e){bgnToastUpdate(t,e.message||String(e),'error')}}
document.getElementById('c').addEventListener('scroll',e=>{const c=e.currentTarget;stick=c.scrollHeight-c.scrollTop-c.clientHeight<100},{passive:true});renderLogs();setInterval(pullLogs,2200);
'''.replace('__INITIAL__', initial)

    return page('Logs', css + body, active='logs', raw_js=js)
...
# v3.9 UI design-system override
CSS += r"""
:root{--bg:#0b0b0d;--panel:#111114;--panel2:#17171b;--panel3:#1d1d22;--hair:#29292f;--hair2:#34343b;--ink:#f5f5f6;--ink2:#b4b4bd;--ink3:#777781;--brand:#f7f7f8;--brandInk:#101012;--ok:#54d98c;--warn:#f1b95b;--bad:#f27285;--r-xs:8px;--r-sm:12px;--r-md:16px;--r-lg:20px;--r-xl:26px;--shadow:0 16px 54px rgba(0,0,0,.26);--ease:cubic-bezier(.2,.8,.2,1)}
[data-theme=light]{--bg:#fafafa;--panel:#fff;--panel2:#f6f6f7;--panel3:#efeff1;--hair:#e4e4e7;--hair2:#d5d5d9;--ink:#161618;--ink2:#606068;--ink3:#96969e;--brand:#171719;--brandInk:#fff;--shadow:0 16px 44px rgba(20,20,25,.08)}
*{box-sizing:border-box}html{background:var(--bg)}body{margin:0;background:var(--bg);color:var(--ink);font:14px/1.55 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;-webkit-font-smoothing:antialiased}a{color:inherit;text-decoration:none}button,input,textarea,select{font:inherit}
::-webkit-scrollbar{width:9px;height:9px}::-webkit-scrollbar-thumb{background:var(--hair2);border:2px solid transparent;background-clip:padding-box;border-radius:999px}
.topbar{height:60px;padding:0 18px;border-bottom:1px solid var(--hair);background:color-mix(in srgb,var(--bg) 88%,transparent);backdrop-filter:blur(18px);display:flex;align-items:center;position:sticky;top:0;z-index:60}
.brand{display:flex;align-items:center;gap:10px;font-weight:680;letter-spacing:-.02em}.brand .dot{width:30px;height:30px;border-radius:10px;background:var(--brand);color:var(--brandInk)}
.shell{display:grid;grid-template-columns:255px minmax(0,1fr);min-height:calc(100vh - 60px)}
.rail{border-right:1px solid var(--hair);padding:16px 12px;background:var(--panel);position:sticky;top:60px;height:calc(100vh - 60px);overflow:auto}.rail::before{content:"Settings";display:block;padding:3px 10px 14px;color:var(--ink3);font-size:11px;font-weight:650}
.rail a{display:flex;align-items:center;gap:10px;min-height:38px;padding:0 11px;margin:2px 0;border-radius:var(--r-sm);color:var(--ink2);font-weight:540;transition:.16s var(--ease)}.rail a:hover{background:var(--panel2);color:var(--ink);transform:translateX(1px)}.rail a.on{background:var(--panel2);color:var(--ink);box-shadow:inset 0 0 0 1px var(--hair)}
.main{padding:32px clamp(20px,4vw,54px);max-width:1420px;width:100%;margin:0 auto}.pagehead{margin:0 0 26px;align-items:flex-start}.pagehead h1{font-size:28px;letter-spacing:-.04em;margin:0 0 5px;font-weight:690}.pagehead p{color:var(--ink3);max-width:700px;margin:0}
.card,.metric,.status-card,.quick,.authcard{background:var(--panel);border:1px solid var(--hair);border-radius:var(--r-lg);box-shadow:0 1px 1px rgba(0,0,0,.04);transition:border-color .16s var(--ease),transform .16s var(--ease),box-shadow .16s var(--ease)}.card{padding:20px}.card:hover{border-color:var(--hair2)}
.metrics,.status-strip{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.metric,.status-card{padding:18px;min-height:124px}.metric .v,.status-card .num{font-size:30px;font-weight:690;letter-spacing:-.05em;margin-top:20px}.metric .k,.status-card .label{font-size:12px;color:var(--ink2)}.metric .s,.status-card .meta{font-size:11px;color:var(--ink3);margin-top:4px}
.btn{min-height:36px;padding:0 14px;border:1px solid var(--hair);border-radius:var(--r-sm);background:var(--panel);color:var(--ink);font-weight:560;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;gap:7px;transition:.15s var(--ease)}.btn:hover{background:var(--panel2);transform:translateY(-1px)}.btn.primary{background:var(--brand);color:var(--brandInk);border-color:transparent}.btn.ghost{background:transparent}
input,textarea,select{border:1px solid var(--hair);background:var(--panel2);color:var(--ink);border-radius:var(--r-sm);outline:0;transition:.16s var(--ease)}input:focus,textarea:focus,select:focus{border-color:var(--hair2);box-shadow:0 0 0 3px color-mix(in srgb,var(--ink) 6%,transparent)}
table{border-collapse:separate;border-spacing:0;width:100%}th{font-size:10px;text-transform:uppercase;letter-spacing:.07em;color:var(--ink3);font-weight:650;text-align:left;padding:10px 12px;border-bottom:1px solid var(--hair)}td{padding:12px;border-bottom:1px solid color-mix(in srgb,var(--hair) 75%,transparent)}tbody tr:hover td{background:color-mix(in srgb,var(--panel2) 58%,transparent)}
.pill{border-radius:999px;padding:4px 8px}.console{background:#09090b!important;border:1px solid var(--hair)!important;border-radius:var(--r-md)!important}.quick-grid{gap:9px}.quick{border-radius:var(--r-md)!important}
@media(max-width:980px){.shell{grid-template-columns:78px minmax(0,1fr)}.rail{padding:14px 8px}.rail::before{display:none}.rail a span:last-child{display:none}.rail a{justify-content:center}.metrics,.status-strip{grid-template-columns:repeat(2,1fr)}}
@media(max-width:640px){.shell{display:block}.rail{position:sticky;top:60px;height:auto;display:flex;overflow:auto;border-right:0;border-bottom:1px solid var(--hair);z-index:40}.rail a span:last-child{display:inline}.rail a{font-size:12px;white-space:nowrap}.main{padding:24px 14px}.metrics,.status-strip{grid-template-columns:1fr}}
@media(prefers-reduced-motion:reduce){*,*::before,*::after{scroll-behavior:auto!important;transition:none!important;animation:none!important}}
"""
def page(title: str, content: str, active: str = "", raw_js: str = "", wide: bool = False) -> str:
    nav = [
        ("dash", "/dashboard", "General", "home"),
        ("chat", "/chat", "Chat", "chat"),
        ("browser", "/browser-view", "Browsers", "browser"),
        ("pool", "/pool", "Network", "network"),
        ("jars", "/jars", "Accounts", "users"),
        ("models", "/models-page", "Models", "models"),
        ("keys", "/api-keys", "API keys", "key"),
        ("errors", "/errors", "Errors", "alert"),
        ("logs", "/logs", "Logs", "logs"),
    ]
    icons = {
        "home": '<svg viewBox="0 0 24 24"><path d="M3 10.8 12 3l9 7.8v9.1a1.1 1.1 0 0 1-1.1 1.1H4.1A1.1 1.1 0 0 1 3 19.9Z"/><path d="M9 21v-7h6v7"/></svg>',
        "chat": '<svg viewBox="0 0 24 24"><path d="M21 12a8 8 0 0 1-8 8H6l-4 2 1.4-4.2A8.5 8.5 0 1 1 21 12Z"/></svg>',
        "browser": '<svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="16" rx="3"/><path d="M3 9h18"/><path d="M7 6.5h.01M10 6.5h.01"/></svg>',
        "network": '<svg viewBox="0 0 24 24"><circle cx="12" cy="5" r="2"/><circle cx="5" cy="18" r="2"/><circle cx="19" cy="18" r="2"/><path d="M12 7v4M7 17l4-4M17 17l-4-4"/></svg>',
        "users": '<svg viewBox="0 0 24 24"><circle cx="9" cy="8" r="3"/><path d="M3 20c.5-4 2.5-6 6-6s5.5 2 6 6"/><circle cx="17" cy="9" r="2"/><path d="M16 15c3.2.1 4.7 1.8 5 5"/></svg>',
        "models": '<svg viewBox="0 0 24 24"><path d="m12 3 8 4.5-8 4.5-8-4.5Z"/><path d="m4 12 8 4.5 8-4.5M4 16.5 12 21l8-4.5"/></svg>',
        ...
        "logs": '<svg viewBox="0 0 24 24"><path d="M5 6h14M5 12h14M5 18h10"/></svg>',
    }
    items = []
    for key, href, label, ico in nav:
        cls = "ow-settings-item active" if active == key else "ow-settings-item"
        items.append(
            f'<a class="{cls}" href="{href}"><span class="ow-nav-icon">{icons[ico]}</span><span>{esc(label)}</span></a>'
        )
    links = "".join(items)
    tpl = r'''<!doctype html>
...
<title>__TITLE__ · Bridgena</title>
<style>
:root{--bg:#201d18;--surface:#27231e;--surface2:#302a24;--surface3:#383129;--line:#4a4036;--line2:#5b4e42;--fg:#f3ede2;--muted:#beb3a5;--dim:#918578;--accent:#d97757;--accent-fg:#fff8f1;--good:#7acb98;--warn:#e0ad67;--bad:#e78882;--shadow:0 24px 80px rgba(25,18,12,.30);--ease:cubic-bezier(.2,.8,.2,1);--serif:"Iowan Old Style","Palatino Linotype","Book Antiqua",Palatino,Charter,Georgia,serif;--sans:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
[data-theme=light]{--bg:#f5efe5;--surface:#fffaf2;--surface2:#eee6da;--surface3:#e7ddcf;--line:#d7cbbc;--line2:#c7b8a6;--fg:#312b25;--muted:#74695e;--dim:#9e9183;--accent:#c76647;--accent-fg:#fffaf4;--shadow:0 24px 70px rgba(72,54,38,.10)}
*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:var(--bg);color:var(--fg)}body{font:14px/1.5 var(--sans);-webkit-font-smoothing:antialiased}button,input,textarea,select{font:inherit}a{color:inherit;text-decoration:none}svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}
.card,.metric,.btn,input,textarea,select,.pill,.ow-settings-item,.console,.toast{corner-shape:squircle}
.ow-settings-app{min-height:100dvh;display:grid;grid-template-columns:282px minmax(0,1fr)}
.ow-settings-nav{position:sticky;top:0;height:100dvh;background:var(--surface);border-right:1px solid var(--line);padding:18px 14px;overflow:auto}
.ow-brand{height:44px;display:flex;align-items:center;gap:11px;padding:0 8px;margin-bottom:24px;font-weight:690;letter-spacing:-.025em}.ow-brandmark{width:31px;height:31px;border-radius:11px;corner-shape:squircle;background:var(--fg);color:var(--bg);display:grid;place-items:center;font-size:11px;font-weight:800}
.ow-section-label{font-size:10px;color:var(--dim);text-transform:uppercase;letter-spacing:.09em;font-weight:700;padding:0 10px 8px}
.ow-settings-item{height:42px;border-radius:13px;display:flex;align-items:center;gap:11px;padding:0 11px;margin:2px 0;color:var(--muted);font-weight:540;transition:.14s var(--ease)}.ow-settings-item:hover{background:var(--surface2);color:var(--fg)}.ow-settings-item.active{background:var(--surface2);color:var(--fg);box-shadow:inset 0 0 0 1px var(--line)}.ow-nav-icon{display:grid;place-items:center;width:21px}.ow-nav-bottom{margin-top:24px;padding-top:14px;border-top:1px solid var(--line)}
.ow-settings-main{min-width:0;padding:34px clamp(20px,4vw,58px) 70px}.ow-settings-inner{width:min(100%,1100px);margin:0 auto}.ow-settings-head{display:flex;align-items:flex-start;gap:14px;margin-bottom:26px}.ow-settings-head h1{margin:0;font:600 30px/1.08 var(--serif);letter-spacing:-.03em}.ow-settings-head p{margin:6px 0 0;color:var(--muted)}.ow-head-actions{margin-left:auto;display:flex;gap:8px}
.card,.metric,.status-card,.quick{background:var(--surface);border:1px solid var(--line);border-radius:20px;corner-shape:squircle;box-shadow:0 1px 0 rgba(255,255,255,.015);overflow:hidden}.card{padding:20px}.card + .card{margin-top:12px}.card h3{margin:0 0 14px;font:600 17px/1.2 var(--serif);letter-spacing:-.01em}
.metrics,.status-strip{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-bottom:14px}.metric,.status-card{min-height:118px;padding:17px}.metric .k,.status-card .label{color:var(--muted);font-size:11px}.metric .v,.status-card .num{font-size:29px;line-height:1;font-weight:690;letter-spacing:-.05em;margin-top:25px}.metric .s,.status-card .meta{color:var(--dim);font-size:10px;margin-top:6px}
.btn{height:36px;border:1px solid var(--line);border-radius:12px;padding:0 13px;background:var(--surface);color:var(--fg);display:inline-flex;align-items:center;justify-content:center;gap:7px;cursor:pointer;font-weight:560;transition:.14s var(--ease)}.btn:hover{background:var(--surface2);border-color:var(--line2)}.btn.primary{background:var(--accent);color:var(--accent-fg);border-color:transparent}.btn.ghost{background:transparent}
input,textarea,select{border:1px solid var(--line);border-radius:12px;background:var(--surface2);color:var(--fg);outline:0;padding:9px 11px}input:focus,textarea:focus,select:focus{border-color:var(--line2);box-shadow:0 0 0 3px color-mix(in srgb,var(--fg) 5%,transparent)}
table{width:100%;border-collapse:separate;border-spacing:0}th{text-align:left;color:var(--dim);font-size:10px;text-transform:uppercase;letter-spacing:.07em;font-weight:700;padding:10px 12px;border-bottom:1px solid var(--line)}td{padding:12px;border-bottom:1px solid color-mix(in srgb,var(--line) 75%,transparent)}tbody tr:last-child td{border-bottom:0}tbody tr:hover td{background:color-mix(in srgb,var(--surface2) 64%,transparent)}
.pill{border-radius:999px;padding:4px 8px}.console{border-radius:16px!important;background:#08080a!important;border:1px solid var(--line)!important;max-width:100%;overflow:auto}.quick{border-radius:16px!important}
.toast-stack{position:fixed;right:16px;bottom:16px;z-index:9999;display:flex;flex-direction:column;gap:8px}.toast{border-radius:14px;background:var(--surface);border:1px solid var(--line);box-shadow:var(--shadow);padding:11px 13px;min-width:240px}
.row,.split{min-width:0}
@media(max-width:980px){.ow-settings-app{grid-template-columns:82px minmax(0,1fr)}.ow-settings-nav{padding:14px 9px}.ow-brand span:last-child,.ow-settings-item span:last-child,.ow-section-label{display:none}.ow-brand{justify-content:center;padding:0}.ow-settings-item{justify-content:center;padding:0}.metrics,.status-strip{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:680px){.ow-settings-app{display:block}.ow-settings-nav{position:sticky;z-index:50;top:0;height:64px;display:flex;align-items:center;overflow-x:auto;overflow-y:hidden;padding:8px;border-right:0;border-bottom:1px solid var(--line)}.ow-brand{margin:0 8px 0 0;height:44px}.ow-brand span:last-child{display:none}.ow-section-label,.ow-nav-bottom{display:none}.ow-settings-item{flex:0 0 42px;width:42px;margin:0 2px}.ow-settings-item span:last-child{display:none}.ow-settings-main{padding:24px 13px 60px}.metrics,.status-strip{grid-template-columns:1fr 1fr}.ow-settings-head{align-items:center}.ow-settings-head h1{font-size:23px}}
@media(max-width:430px){.metrics,.status-strip{grid-template-columns:1fr}.ow-head-actions{display:none}}

/* v3.9.2: complete settings component system */
.muted{color:var(--muted)}.mono{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}.small{font-size:11px}.spacer{flex:1}.empty{padding:32px!important;text-align:center;color:var(--muted)}
.row{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.grid{display:grid;gap:12px}.split{display:grid;grid-template-columns:minmax(0,1fr) minmax(300px,.62fr);gap:14px}.section-grid{display:grid;grid-template-columns:minmax(0,1.35fr) minmax(320px,.65fr);gap:14px;margin-top:14px}
.pagehead{display:flex;align-items:flex-start;gap:14px;margin:0 0 22px}.pagehead>div:first-child{min-width:0}.pagehead .row,.pagehead>.actionbar{margin-left:auto}.pagehead h1{margin:0 0 5px;font-size:27px;letter-spacing:-.045em}.pagehead p{margin:0;color:var(--muted)}
.card,.metric,.status-card,.quick,.settings-section,.action-tile,.account-card,.console,.table-wrap,.dropbox,.tab-panel{border-radius:30px;corner-shape:squircle}.btn,input,textarea,select,.tab,.pill,.badge-num{corner-shape:squircle}
.settings-stack{display:grid;gap:14px}.settings-section{background:var(--surface);border:1px solid var(--line);overflow:hidden}.settings-section-head{padding:20px 22px 15px;border-bottom:1px solid var(--line)}.settings-section-head h2{font-size:15px;margin:0 0 4px;letter-spacing:-.02em}.settings-section-head p{margin:0;color:var(--muted);font-size:12px}.settings-section-body{padding:8px}
.setting-row{min-height:66px;padding:12px 14px;display:flex;align-items:center;gap:16px;border-radius:22px;corner-shape:squircle;transition:.14s var(--ease)}.setting-row:hover{background:var(--surface2)}.setting-copy{min-width:0}.setting-title{font-weight:600}.setting-desc{font-size:11px;color:var(--muted);margin-top:3px}.setting-value{margin-left:auto;display:flex;align-items:center;gap:8px;white-space:nowrap}
.overview-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.overview-tile{min-height:136px;background:var(--surface);border:1px solid var(--line);border-radius:34px;corner-shape:squircle;padding:19px;display:flex;flex-direction:column}.overview-k{font-size:11px;color:var(--muted)}.overview-v{margin-top:auto;font:600 35px/1 var(--serif);letter-spacing:-.035em}.overview-meta{font-size:10px;color:var(--dim);margin-top:6px}
.quick-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px!important}.quick,.action-tile{display:flex!important;flex-direction:column;align-items:flex-start;justify-content:center;gap:4px;min-height:88px;padding:15px!important;background:var(--surface2);border:1px solid var(--line)!important;color:var(--fg);text-decoration:none}.quick:hover,.action-tile:hover{background:var(--surface3);border-color:var(--line2)!important}.quick b,.action-tile b{font-size:13px}.quick span,.action-tile span{font-size:11px;color:var(--muted);line-height:1.4}
.kv{display:flex;align-items:center;gap:12px;justify-content:space-between;min-height:40px;padding:8px 2px;border-bottom:1px solid var(--line);font-size:12px}.kv:last-child{border-bottom:0}.eyebrow{font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:var(--dim);font-weight:700}.badge-num{display:inline-flex;align-items:center;min-height:28px;padding:0 9px;border:1px solid var(--line);border-radius:12px;background:var(--surface2);font-size:11px}.actionbar{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.table-wrap{border:1px solid var(--line);overflow:auto;background:var(--surface)}.table-wrap table{min-width:680px}
.proxy-add{display:grid;grid-template-columns:minmax(0,1.4fr) minmax(340px,.6fr);gap:14px}.tabs{display:flex;gap:5px;padding:5px;background:var(--surface2);border-radius:18px;corner-shape:squircle}.tab{height:34px;border:0;border-radius:12px;background:transparent;color:var(--muted);padding:0 11px;cursor:pointer}.tab.on{background:var(--surface);color:var(--fg);box-shadow:0 1px 2px rgba(0,0,0,.12)}.tab-panel{margin-top:12px}.dropbox{border:1px dashed var(--line2);padding:14px;background:var(--surface2)}.helper{font-size:11px;color:var(--muted);margin-top:8px}.dotlive{display:inline-block;width:8px;height:8px;border-radius:99px;background:var(--good);box-shadow:0 0 0 4px color-mix(in srgb,var(--good) 12%,transparent)}
.pill{display:inline-flex;align-items:center;min-height:24px;padding:0 8px;border-radius:999px;font-size:10px;font-weight:650;border:1px solid var(--line);background:var(--surface2)}.pill.ok{color:var(--good);border-color:color-mix(in srgb,var(--good) 28%,var(--line));background:color-mix(in srgb,var(--good) 8%,var(--surface))}.pill.warn{color:var(--warn);border-color:color-mix(in srgb,var(--warn) 28%,var(--line));background:color-mix(in srgb,var(--warn) 8%,var(--surface))}.pill.bad{color:var(--bad);border-color:color-mix(in srgb,var(--bad) 28%,var(--line));background:color-mix(in srgb,var(--bad) 8%,var(--surface))}.pill.idle{color:var(--muted)}
.btn{border-radius:15px;corner-shape:squircle}.btn.sm{height:32px;min-height:32px;padding:0 10px;font-size:11px}.btn.danger{color:var(--bad);border-color:color-mix(in srgb,var(--bad) 28%,var(--line))}input,textarea,select{border-radius:15px;corner-shape:squircle}.card{border-radius:30px!important;corner-shape:squircle}.console{border-radius:24px!important;corner-shape:squircle}
@media(max-width:1100px){.overview-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.section-grid,.split,.proxy-add{grid-template-columns:1fr}}
@media(max-width:620px){.overview-grid,.quick-grid{grid-template-columns:1fr}.pagehead{display:block}.pagehead .row,.pagehead>.actionbar{margin:12px 0 0}.setting-row{align-items:flex-start;flex-direction:column}.setting-value{margin-left:0}}


/* v3.9.3 warm editorial polish */
.ow-brandmark{background:var(--accent);color:var(--accent-fg);box-shadow:0 8px 24px color-mix(in srgb,var(--accent) 18%,transparent)}
.ow-settings-item{transition:background .18s var(--ease),color .18s var(--ease),transform .18s var(--ease)}
.ow-settings-item:hover{transform:translateX(2px)}
.btn{transition:background .18s var(--ease),border-color .18s var(--ease),transform .18s var(--ease),box-shadow .18s var(--ease)}
.btn:hover{transform:translateY(-1px);box-shadow:0 7px 20px rgba(30,20,12,.14)}
.btn.primary{background:var(--accent);color:var(--accent-fg)}
.card,.overview-tile,.settings-section,.quick{transition:transform .18s var(--ease),border-color .18s var(--ease),background .18s var(--ease)}
.card:hover,.overview-tile:hover,.settings-section:hover{transform:translateY(-1px);border-color:var(--line2)}
.pagehead h1,.hero h1,.setting-title,.overview-k{font-family:var(--serif)}

@media(prefers-reduced-motion:reduce){*{animation:none!important;transition:none!important;scroll-behavior:auto!important}}
</style></head><body>
<div class="ow-settings-app"><aside class="ow-settings-nav">
<a class="ow-brand" href="/dashboard"><span class="ow-brandmark">B</span><span>Bridgena</span></a><div class="ow-section-label">Admin settings</div>__LINKS__
<div class="ow-nav-bottom"><a class="ow-settings-item" href="/chat"><span class="ow-nav-icon">__CHAT_ICON__</span><span>Back to chat</span></a><a class="ow-settings-item" href="/logout"><span class="ow-nav-icon">__LOGOUT_ICON__</span><span>Sign out</span></a></div>
</aside><main class="ow-settings-main"><div class="ow-settings-inner"><div class="ow-settings-head"><div><h1>__TITLE__</h1><p>Bridgena control plane</p></div><div class="ow-head-actions"><button class="btn ghost" onclick="bgnTheme()">Theme</button><a class="btn" href="/chat">Open chat</a></div></div>__CONTENT__</div></main></div>
<div id="bgn-toasts" class="toast-stack"></div>
<script>document.documentElement.dataset.theme=localStorage.getItem('bgn.theme')||'dark';function bgnTheme(){const r=document.documentElement,n=r.dataset.theme==='light'?'dark':'light';r.dataset.theme=n;localStorage.setItem('bgn.theme',n)}function bgnToast(message,type='ok'){const root=document.getElementById('bgn-toasts'),el=document.createElement('div');el.className='toast '+type;el.textContent=message;root.appendChild(el);setTimeout(()=>el.remove(),3200);return el}function bgnToastUpdate(el,message,type='ok'){if(!el)return;el.textContent=message;el.className='toast '+type}__RAW_JS__</script>
</body></html>'''
    chat_icon = icons["chat"]
    logout_icon = '<svg viewBox="0 0 24 24"><path d="M10 5H5a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h5"/><path d="m15 8 4 4-4 4M19 12H9"/></svg>'
    return (tpl.replace("__TITLE__", esc(title)).replace("__LINKS__", links).replace("__CHAT_ICON__", chat_icon).replace("__LOGOUT_ICON__", logout_icon).replace("__CONTENT__", content).replace("__RAW_JS__", raw_js))

def login_page(err: str = "") -> str:
    tpl = r'''<!doctype html><html data-theme="dark"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="dark light"><title>Sign in · Bridgena</title>
<style>
:root{--bg:#09090b;--surface:#111114;--surface2:#18181c;--line:#2a2a30;--fg:#f4f4f5;--muted:#a1a1aa;--bad:#fb7185;--ease:cubic-bezier(.2,.8,.2,1)}[data-theme=light]{--bg:#fafafa;--surface:#fff;--surface2:#f4f4f5;--line:#e4e4e7;--fg:#18181b;--muted:#71717a;--bad:#e34c65}
*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:var(--bg);color:var(--fg)}body{font:14px/1.5 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;-webkit-font-smoothing:antialiased}.auth{min-height:100dvh;display:grid;grid-template-columns:minmax(0,1fr) minmax(430px,560px)}
.hero{position:relative;overflow:hidden;background:linear-gradient(145deg,#101014,#070709);padding:46px;display:flex;flex-direction:column;justify-content:space-between;border-right:1px solid var(--line)}.hero:before{content:"";position:absolute;width:720px;height:720px;border-radius:50%;background:radial-gradient(circle,rgba(255,255,255,.07),transparent 66%);left:-260px;top:-300px}
.brand{position:relative;display:flex;align-items:center;gap:11px;font-weight:700}.mark{width:34px;height:34px;border-radius:12px;corner-shape:squircle;background:#f4f4f5;color:#111113;display:grid;place-items:center;font-weight:800}.hero-copy{position:relative;max-width:560px}.hero-copy h2{font-size:44px;line-height:1.04;letter-spacing:-.055em;margin:0 0 15px;font-weight:680}.hero-copy p{font-size:15px;color:#9d9da7;max-width:480px}
.login-side{display:grid;place-items:center;padding:30px;background:var(--bg)}.login-box{width:min(390px,100%);animation:enter .36s var(--ease)}.login-box h1{font-size:27px;letter-spacing:-.045em;margin:0 0 7px}.login-box>p{color:var(--muted);margin:0 0 28px}.form{border:1px solid var(--line);border-radius:30px;corner-shape:squircle;background:var(--surface);padding:22px}label{display:block;font-size:12px;color:var(--muted);margin-bottom:7px}
input{width:100%;height:45px;border:1px solid var(--line);border-radius:13px;corner-shape:squircle;background:var(--surface2);color:var(--fg);outline:0;padding:0 13px;font:inherit}input:focus{box-shadow:0 0 0 3px rgba(255,255,255,.05)}button{width:100%;height:45px;border:0;border-radius:13px;corner-shape:squircle;background:var(--fg);color:var(--bg);font:650 14px inherit;margin-top:12px;cursor:pointer}.err{margin-bottom:12px;border:1px solid color-mix(in srgb,var(--bad) 25%,transparent);background:color-mix(in srgb,var(--bad) 9%,transparent);color:var(--bad);padding:10px 11px;border-radius:12px;corner-shape:squircle}.foot{color:var(--muted);font-size:11px;margin-top:16px}
@keyframes enter{from{opacity:0;transform:translateY(7px)}to{opacity:1;transform:none}}@media(max-width:820px){.auth{grid-template-columns:1fr}.hero{display:none}.login-side{min-height:100dvh;padding:22px}}@media(prefers-reduced-motion:reduce){*{animation:none!important;transition:none!important}}
</style></head><body><div class="auth"><section class="hero"><div class="brand"><span class="mark">B</span>Bridgena</div><div class="hero-copy"><h2>Your AI gateway,<br>under control.</h2><p>Manage models, browser workers, accounts, networking and API compatibility from one private workspace.</p></div><div style="position:relative;color:#73737d;font-size:11px">Control plane</div></section>
<section class="login-side"><div class="login-box"><h1>Welcome back</h1><p>Sign in to continue to your workspace.</p><form class="form" method="post" action="/login">__ERR__<label for="password">Admin password</label><input id="password" name="password" type="password" autocomplete="current-password" autofocus placeholder="Enter your password"><button type="submit">Continue</button></form><div class="foot">Private Bridgena administration</div></div></section></div><script>document.documentElement.dataset.theme=localStorage.getItem('bgn.theme')||'dark'</script></body></html>'''
    return tpl.replace("__ERR__", f'<div class="err">{esc(err)}</div>' if err else "")
...
def dashboard_page(overview: dict) -> str:
    ...
    healthy_pct=round((m['alive']/m['pool_total']*100) if m['pool_total'] else 0)
    prow=''.join(
        f"<tr><td class='mono'>{esc(r.get('display') or '—')}</td><td>{_verdict_pill(r.get('verdict','unknown'))}</td><td class='muted'>{esc(r.get('latency') or '—')}{'ms' if r.get('latency') else ''}</td><td class='muted'>{esc((r.get('why') or '—')[:90])}</td></tr>"
        for r in pool[:8]
    ) or "<tr><td colspan='4' class='empty'>No network telemetry yet.</td></tr>"
    logl=''.join(f"<div class='{esc(x.get('lvl','INFO'))}'>{esc(x.get('line') or x.get('message') or '')}</div>" for x in overview.get('logtail',[])[-28:])
    content=f"""
<div class='settings-stack'>
  <div class='overview-grid'>
    <div class='overview-tile'><div class='overview-k'>Healthy exits</div><div class='overview-v'>{m['alive']}</div><div class='overview-meta'>{healthy_pct}% of {m['pool_total']} configured routes</div></div>
    <div class='overview-tile'><div class='overview-k'>Live keepers</div><div class='overview-v'>{m['keepers_live']}</div><div class='overview-meta'>{m['jars_ok']} authenticated accounts</div></div>
    <div class='overview-tile'><div class='overview-k'>Distinct routes</div><div class='overview-v'>{distinct_routes}</div><div class='overview-meta'>Current account allocation</div></div>
    <div class='overview-tile'><div class='overview-k'>Models</div><div class='overview-v'>{m['models']}</div><div class='overview-meta'>Available through compatibility API</div></div>
  </div>
  <section class='settings-section'><div class='settings-section-head'><h2>System</h2><p>Current Bridgena runtime and capacity.</p></div><div class='settings-section-body'>
    <div class='setting-row'><div class='setting-copy'><div class='setting-title'>Network availability</div><div class='setting-desc'>Live, reachable exits in the configured pool.</div></div><div class='setting-value'><span class='pill ok'>{m['alive']} healthy</span><a class='btn sm' href='/pool'>Manage</a></div></div>
    <div class='setting-row'><div class='setting-copy'><div class='setting-title'>Browser workers</div><div class='setting-desc'>Persistent keeper sessions currently available.</div></div><div class='setting-value'><span class='pill {'ok' if m['keepers_live'] else 'warn'}'>{m['keepers_live']} live</span><a class='btn sm' href='/browser-view'>Inspect</a></div></div>
    <div class='setting-row'><div class='setting-copy'><div class='setting-title'>Accounts</div><div class='setting-desc'>Authenticated accounts ready for the scheduler.</div></div><div class='setting-value'><span>{m['jars_ok']} / {m['jars_total']}</span><a class='btn sm' href='/jars'>Accounts</a></div></div>
    <div class='setting-row'><div class='setting-copy'><div class='setting-title'>Model catalog</div><div class='setting-desc'>Published model compatibility inventory.</div></div><div class='setting-value'><span>{m['models']}</span><a class='btn sm' href='/models-page'>Models</a></div></div>
  </div></section>
  <div class='section-grid'>
    <section class='settings-section'><div class='settings-section-head'><h2>Network health</h2><p>Recent exit telemetry.</p></div><div class='table-wrap' style='border:0;border-radius:0'><table><thead><tr><th>Exit</th><th>State</th><th>RTT</th><th>Diagnosis</th></tr></thead><tbody>{prow}</tbody></table></div></section>
    <section class='settings-section'><div class='settings-section-head'><h2>Quick actions</h2><p>Common administration tasks.</p></div><div class='settings-section-body'><div class='quick-grid'>
      <a class='action-tile' href='/pool'><b>Network</b><span>Add, scan and manage exits</span></a>
      <a class='action-tile' href='/jars'><b>Accounts</b><span>Manage keepers and personas</span></a>
      <a class='action-tile' href='/api-keys'><b>API keys</b><span>Create and revoke credentials</span></a>
      <a class='action-tile' href='/models-page'><b>Models</b><span>Inspect compatibility catalog</span></a>
    </div></div></section>
  </div>
  <section class='settings-section'><div class='settings-section-head'><div class='row'><div><h2>Runtime activity</h2><p>Recent control-plane events.</p></div><span class='spacer'></span><a class='btn sm' href='/logs'>Open logs</a></div></div><div style='padding:12px'><div class='console' id='cons' style='height:260px'>{logl}</div></div></section>
</div>"""
    js=r"""async function consRefresh(){try{const r=await fetch('/debug-logs/data',{cache:'no-store'});if(!r.ok)return;const d=await r.json(),c=document.getElementById('cons');c.innerHTML=d.slice(-28).map(x=>'<div class=\"'+(x.lvl||'INFO')+'\">'+String(x.line||x.message||'').replace(/[&<>]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[m]))+'</div>').join('');c.scrollTop=c.scrollHeight}catch(e){}}setInterval(consRefresh,3500);"""
    return page('General',content,'dash',js)
...
[data-theme=light]{--bg:#f6f0e7;--sidebar:#fffaf2;--surface:#eee5d9;--surface2:#e7dccf;--line:#d7cbbc;--fg:#312b25;--muted:#74695e;--dim:#9e9183;--good:#2f8f62;--warn:#a36f2c;--bad:#b95f5c;--brand:#c76647;--shadow:0 24px 70px rgba(72,54,38,.12)}
*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;background:var(--bg);color:var(--fg)}body{font:14px/1.55 var(--sans);-webkit-font-smoothing:antialiased}button,input,textarea{font:inherit}button{color:inherit}svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}
button,.ow-model-picker,.ow-composer,.ow-user-bubble,.ow-runtime,.ow-suggestion,.ow-thread,.ow-search{corner-shape:squircle}
.ow-app{height:100dvh;display:grid;grid-template-columns:260px minmax(0,1fr);overflow:hidden}.ow-sidebar{min-width:0;height:100dvh;display:grid;grid-template-rows:auto auto minmax(0,1fr) auto;background:var(--sidebar);border-right:1px solid var(--line)}
.ow-side-head{height:58px;padding:0 13px;display:flex;align-items:center;gap:10px}.ow-logo{width:33px;height:33px;border-radius:13px;corner-shape:squircle;background:var(--brand);color:#fff8f1;display:grid;place-items:center;font:700 15px var(--serif);box-shadow:0 8px 24px color-mix(in srgb,var(--brand) 22%,transparent)}.ow-title{font-weight:680;letter-spacing:-.02em}.ow-side-tools{padding:5px 8px 8px}.ow-new{width:100%;height:40px;border:0;border-radius:12px;background:transparent;display:flex;align-items:center;gap:10px;padding:0 10px;cursor:pointer;font-weight:560}.ow-new:hover{background:var(--surface)}
.ow-search{height:35px;border:1px solid transparent;border-radius:11px;background:transparent;display:flex;align-items:center;gap:8px;padding:0 10px;color:var(--muted);margin-top:3px}.ow-search:focus-within{background:var(--surface);border-color:var(--line)}.ow-search input{min-width:0;width:100%;border:0;outline:0;background:transparent;color:var(--fg);font-size:12px}
.ow-history{min-height:0;overflow-y:auto;padding:4px 7px 14px}.ow-history-label{padding:12px 9px 6px;color:var(--dim);font-size:10px;font-weight:700}.ow-thread{width:100%;min-height:37px;border:0;border-radius:11px;background:transparent;color:var(--muted);padding:8px 10px;text-align:left;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin:1px 0}.ow-thread:hover{background:var(--surface);color:var(--fg)}.ow-thread.active{background:var(--surface);color:var(--fg)}
.ow-side-foot{padding:8px;border-top:1px solid var(--line)}.ow-admin{height:41px;border-radius:12px;corner-shape:squircle;display:flex;align-items:center;gap:10px;padding:0 10px;color:var(--muted);text-decoration:none}.ow-admin:hover{background:var(--surface);color:var(--fg)}
...
.ow-user{display:flex;justify-content:flex-end;padding-left:17%}.ow-user-bubble{max-width:min(80%,690px);border-radius:28px 28px 10px 28px;corner-shape:squircle;background:var(--surface);border:1px solid color-mix(in srgb,var(--line) 76%,transparent);padding:11px 15px;font:500 14px/1.62 var(--sans);word-break:break-word;box-shadow:0 6px 18px rgba(22,15,10,.08)}
.ow-composer-zone{position:relative;z-index:15;padding:8px 18px max(18px,env(safe-area-inset-bottom));background:linear-gradient(to bottom,transparent 0,var(--bg) 26px,var(--bg) 100%)}.ow-composer{width:min(100%,52rem);margin:0 auto;border:1px solid var(--line);border-radius:34px;corner-shape:squircle;background:color-mix(in srgb,var(--sidebar) 92%,var(--surface));box-shadow:0 14px 42px rgba(25,18,12,.18);padding:11px 11px 10px;transition:border-color .2s var(--ease),box-shadow .2s var(--ease),transform .2s var(--ease)}.ow-composer:focus-within{border-color:color-mix(in srgb,var(--brand) 55%,var(--line));box-shadow:0 16px 48px rgba(25,18,12,.22),0 0 0 4px color-mix(in srgb,var(--brand) 9%,transparent);transform:translateY(-1px)}.ow-composer textarea{display:block;width:100%;height:46px;min-height:46px;max-height:180px;resize:none;overflow-y:auto;border:0;outline:0;background:transparent;color:var(--fg);padding:7px 7px 5px;line-height:1.55}.ow-compose-foot{height:36px;display:flex;align-items:center;gap:7px;color:var(--dim);font-size:10px;padding-left:5px}.ow-send{margin-left:auto;width:36px;height:36px;border:0;border-radius:14px;corner-shape:squircle;background:var(--brand);color:#fff8f1;display:grid;place-items:center;cursor:pointer;transition:transform .18s var(--ease),box-shadow .18s var(--ease)}.ow-send:hover{transform:translateY(-1px) scale(1.035);box-shadow:0 8px 20px color-mix(in srgb,var(--brand) 24%,transparent)}.ow-send.stop{background:var(--bad);color:white}
.ow-runtime{position:fixed;z-index:120;right:13px;top:66px;bottom:13px;width:min(520px,calc(100vw - 26px));display:none;flex-direction:column;background:var(--sidebar);border:1px solid var(--line);border-radius:20px;box-shadow:var(--shadow);padding:12px;animation:owslide .18s var(--ease)}.ow-runtime.open{display:flex}.ow-runtime-head{display:flex;align-items:center;height:38px;padding:0 3px 8px}.ow-runtime pre{min-height:0;flex:1;overflow:auto;margin:0;border:1px solid var(--line);border-radius:14px;background:var(--bg);padding:11px;color:var(--muted);white-space:pre-wrap;font:11px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace}.ow-sidebar-backdrop{display:none}
@keyframes owpop{from{opacity:0;transform:translateY(-5px) scale(.985)}to{opacity:1;transform:none}}@keyframes owmsg{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:none}}@keyframes owslide{from{opacity:0;transform:translateX(9px)}to{opacity:1;transform:none}}@keyframes owPulse{0%,100%{box-shadow:0 0 0 0 color-mix(in srgb,var(--warn) 0%,transparent)}50%{box-shadow:0 0 0 7px color-mix(in srgb,var(--warn) 10%,transparent)}}.ow-status-dot.busy{background:var(--warn)!important;animation:owPulse 1.4s ease-in-out infinite}.ow-thread,.ow-suggestion,.ow-icon-btn,.ow-model-btn{transition:background .18s var(--ease),color .18s var(--ease),transform .18s var(--ease)}.ow-thread:hover,.ow-suggestion:hover{transform:translateX(2px)}
@media(max-width:760px){.ow-app{grid-template-columns:1fr}.ow-sidebar{position:fixed;z-index:200;left:0;top:0;bottom:0;width:min(285px,86vw);transform:translateX(-102%);transition:.18s var(--ease);box-shadow:var(--shadow)}.ow-sidebar.open{transform:none}.ow-sidebar-backdrop{display:block;position:fixed;z-index:190;inset:0;background:rgba(0,0,0,.42);opacity:0;pointer-events:none;transition:.18s}.ow-sidebar-backdrop.open{opacity:1;pointer-events:auto}.ow-mobile-menu{display:grid}.ow-feed{padding:24px 14px 26px}.ow-empty{min-height:calc(100dvh - 190px)}.ow-empty h1{font-size:28px}.ow-suggestions{grid-template-columns:1fr}.ow-user{padding-left:7%}.ow-user-bubble{max-width:91%}.ow-composer-zone{padding-left:9px;padding-right:9px}.ow-status span{display:none}.ow-model-btn{max-width:58vw}}
...
renderThreads();renderConversation();refreshSignal();document.addEventListener('click',e=>{if(!$('picker').contains(e.target)&&!$('modelBtn').contains(e.target))$('picker').classList.remove('open')});
    ...
    return tpl.replace("__MODELS__", mj).replace("__DEFAULT__", dj)
...
if _V3_VNC_DISPLAY and not _V3_VNC_DISPLAY.startswith(':') and not _V3_VNC_DISPLAY.startswith('/'):
    _V3_VNC_DISPLAY=':'+_V3_VNC_DISPLAY
...
async def _v3_vnc_start():
    ...
    if current_display and not current_display.startswith(':') and not current_display.startswith('/'):
        normalized = ':' + current_display
        log('WARN', f'Normalizing invalid DISPLAY={current_display!r} -> {normalized!r}')
        current_display = normalized
    ...
    try:
        ...
        display_usable = bool(current_display and os.path.exists(display_socket))

        if need_display and not display_usable:
            ...
            if not os.path.exists(display_socket):
            ...
            os.environ['DISPLAY'] = display
            log('INFO', f'Headed keeper mode reusing DISPLAY={display}')
...
def _tool_openai_defs(body: dict) -> list:
    out = []
    for raw in body.get("tools") or []:
        ...
        out.append({
            "name": name,
            "description": str(fn.get("description") or "")[:6000],
            "parameters": fn.get("parameters") if isinstance(fn.get("parameters"), dict) else
                          (fn.get("input_schema") if isinstance(fn.get("input_schema"), dict) else
                           {"type": "object", "properties": {}}),
        })
    return out


def _tool_anthropic_defs(body: dict) -> list:
    out = []
    for raw in body.get("tools") or []:
        if not isinstance(raw, dict):
            continue
        ...
        out.append({
            "name": name,
            "description": str(raw.get("description") or "")[:6000],
...
def _openai_has_tool_context(body: dict) -> bool:
    if _tool_openai_defs(body):
        return True
    for message in body.get("messages") or []:
...
def _anthropic_has_tool_context(body: dict) -> bool:
    if _tool_anthropic_defs(body):
        return True
    for message in body.get("messages") or []:
        if not isinstance(message, dict):
            continue
...
def _tool_choice_text(body: dict, protocol: str) -> str:
    ...
    if protocol == "anthropic":
        ...
        if kind == "tool":
            ...
            return f"required tool: {name}" if name else "required"
        if kind == "any":
            return "required"
        return kind
...
def _tool_runtime_system_context(body: dict, protocol: str) -> str:
    parts = []
    if protocol == "anthropic":
        base = _anthropic_system_context(body)
        if base:
            parts.append(base)
    else:
        base = _openai_system_context(body)
        if base:
            parts.append(base)
        for message in body.get("messages") or []:
            if not isinstance(message, dict) or str(message.get("role") or "").lower() != "developer":
                continue
            value = _openai_text_content(message.get("content", "")).strip()
            if value and value not in parts:
                parts.append(value)
...
def _render_openai_tool_message(message: dict) -> str:
    role = str(message.get("role") or "").strip().lower()
    content = _openai_text_content(message.get("content", "")).strip()
    ...
    for message in body.get("messages") or []:
        ...
        for block in blocks:
            ...
            if typ == "text":
                ...
                cid = str(block.get("tool_use_id") or "").strip()
                name = known_calls.get(cid, "")
                result = block.get("content")
                if isinstance(result, list):
                    result_text = _openai_text_content(result)
...
def _bounded_tool_transcript(rows: list) -> str:
...
def _allowed_tool_names(body: dict, protocol: str) -> set:
    defs = _tool_anthropic_defs(body) if protocol == "anthropic" else _tool_openai_defs(body)
    return {str(item.get("name") or "") for item in defs if item.get("name")}


def _normalize_parsed_tool_calls(value, body: dict, protocol: str) -> list:
    allowed = _allowed_tool_names(body, protocol)
    if not allowed:
        return []
    if isinstance(value, dict):
        calls = value.get("tool_calls")
        if calls is None and isinstance(value.get("tool_call"), dict):
            calls = [value.get("tool_call")]
        if calls is None and value.get("name"):
            calls = [value]
...
def _format_conversation_prompt(body: dict) -> str:
    if _openai_has_tool_context(body):
        return _openai_tool_prompt(body)
    return _disposable_context_prompt(body)


def _anthropic_prompt(body: dict) -> str:
    if _anthropic_has_tool_context(body):
        return _anthropic_tool_prompt(body)
    return _disposable_context_prompt(body)
    ...
    try:
        ...
        return JSONResponse({
            ...
            "usage": {
                ...
                "total_tokens": _rough_tokens(prompt) + _rough_tokens(acc),
            },
        })
    finally:
...
async def openai_stream(body: dict, keyinfo: dict):
    if _openai_has_tool_context(body):
        return await _openai_tool_stream(body, keyinfo)
...
async def chat_completions(request: Request):
    ...
    if not body.get("stream", True):
        if _openai_has_tool_context(body):
            return await _openai_tool_nonstream(body, keyinfo)
...
async def anthropic_count_tokens(request: Request):
    ...
    system = (_tool_runtime_system_context(body, "anthropic")
              if _anthropic_has_tool_context(body) else _anthropic_system_context(body))
    return JSONResponse({"input_tokens": _rough_tokens((prompt or "") + "\n" + (system or ""))})
...
async def anthropic_messages(request: Request):
    ...
    if unsupported_blocks:
        return _anthropic_error(
            400,
            "Browser text mode received a non-text content block it cannot represent.",
            source="anthropic_local_nontext",
        )
    prompt = _anthropic_prompt(body)
    if not prompt:
        raise HTTPException(status_code=400, detail="no user message")
...
header b{font-size:14px}.muted{color:#a1a1aa}.wrap{padding:18px;display:grid;grid-template-columns:290px minmax(0,1fr);gap:16px;height:calc(100vh - 64px)}
.panel{border:1px solid #27272a;border-radius:30px;corner-shape:squircle;background:#0c0c0e;overflow:hidden}
.side{padding:12px;overflow:auto}.session{width:100%;text-align:left;padding:10px;margin-bottom:7px;border:1px solid #27272a;border-radius:18px;corner-shape:squircle;background:#18181b;color:#fafafa;cursor:pointer}
...
<body><header><b>Bridgena</b><span class="muted">Browser workers</span><span style="flex:1"></span><a style="color:#fafafa" href="/vnc">VNC</a><a style="color:#fafafa;margin-left:12px" href="/dashboard">Admin settings</a></header>
