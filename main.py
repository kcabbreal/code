...
BUILD_STAMP = os.environ.get("BRIDGENA_BUILD", "v3.9.2-full-ui-rebuild")
...
<style>
:root{--bg:#09090b;--surface:#0f0f12;--surface2:#151519;--surface3:#1c1c21;--line:#27272d;--line2:#34343b;--fg:#f4f4f5;--muted:#a1a1aa;--dim:#71717a;--accent:#f4f4f5;--accent-fg:#111113;--good:#4ade80;--warn:#fbbf24;--bad:#fb7185;--shadow:0 24px 80px rgba(0,0,0,.34);--ease:cubic-bezier(.2,.8,.2,1)}
[data-theme=light]{--bg:#f7f7f8;--surface:#fff;--surface2:#f4f4f5;--surface3:#ededf0;--line:#e4e4e7;--line2:#d4d4d8;--fg:#18181b;--muted:#71717a;--dim:#a1a1aa;--accent:#18181b;--accent-fg:#fff;--shadow:0 24px 70px rgba(20,20,30,.10)}
*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:var(--bg);color:var(--fg)}body{font:14px/1.5 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;-webkit-font-smoothing:antialiased}button,input,textarea,select{font:inherit}a{color:inherit;text-decoration:none}svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}
...
.ow-settings-main{min-width:0;padding:34px clamp(20px,4vw,58px) 70px}.ow-settings-inner{width:min(100%,1100px);margin:0 auto}.ow-settings-head{display:flex;align-items:flex-start;gap:14px;margin-bottom:26px}.ow-settings-head h1{margin:0;font-size:27px;letter-spacing:-.045em;font-weight:690}.ow-settings-head p{margin:6px 0 0;color:var(--muted)}.ow-head-actions{margin-left:auto;display:flex;gap:8px}
.card,.metric,.status-card,.quick{background:var(--surface);border:1px solid var(--line);border-radius:20px;corner-shape:squircle;box-shadow:0 1px 0 rgba(255,255,255,.015);overflow:hidden}.card{padding:20px}.card + .card{margin-top:12px}.card h3{margin:0 0 14px;font-size:14px;letter-spacing:-.015em}
...
.overview-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.overview-tile{min-height:136px;background:var(--surface);border:1px solid var(--line);border-radius:34px;corner-shape:squircle;padding:19px;display:flex;flex-direction:column}.overview-k{font-size:11px;color:var(--muted)}.overview-v{margin-top:auto;font-size:32px;font-weight:700;letter-spacing:-.055em}.overview-meta{font-size:10px;color:var(--dim);margin-top:6px}
...
<style>
:root{--bg:#09090b;--sidebar:#101013;--surface:#141417;--surface2:#1b1b20;--line:#29292f;--fg:#f4f4f5;--muted:#a1a1aa;--dim:#71717a;--good:#4ade80;--warn:#fbbf24;--bad:#fb7185;--shadow:0 24px 80px rgba(0,0,0,.38);--ease:cubic-bezier(.2,.8,.2,1)}
[data-theme=light]{--bg:#fff;--sidebar:#f7f7f8;--surface:#f4f4f5;--surface2:#ececef;--line:#e3e3e7;--fg:#18181b;--muted:#71717a;--dim:#a1a1aa;--good:#169b5b;--warn:#a76b08;--bad:#df4a63;--shadow:0 24px 70px rgba(20,20,30,.11)}
*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;background:var(--bg);color:var(--fg)}body{font:14px/1.55 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;-webkit-font-smoothing:antialiased}button,input,textarea{font:inherit}button{color:inherit}svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}
...
.ow-side-head{height:58px;padding:0 13px;display:flex;align-items:center;gap:10px}.ow-logo{width:31px;height:31px;border-radius:11px;corner-shape:squircle;background:var(--fg);color:var(--bg);display:grid;place-items:center;font-size:10px;font-weight:800}.ow-title{font-weight:680;letter-spacing:-.02em}.ow-side-tools{padding:5px 8px 8px}.ow-new{width:100%;height:40px;border:0;border-radius:12px;background:transparent;display:flex;align-items:center;gap:10px;padding:0 10px;cursor:pointer;font-weight:560}.ow-new:hover{background:var(--surface)}
...
.ow-message{width:100%;margin:0 0 34px;animation:owmsg .17s var(--ease)}.ow-assistant{display:grid;grid-template-columns:32px minmax(0,1fr);gap:13px}.ow-avatar{width:30px;height:30px;border-radius:11px;corner-shape:squircle;background:var(--surface);display:grid;place-items:center;font-size:9px;font-weight:800}.ow-msg-head{font-size:11px;color:var(--dim);font-weight:650;margin:4px 0 5px}.ow-msg-body{font-size:15px;line-height:1.74;word-break:break-word;min-width:0}.ow-msg-body pre{max-width:100%;overflow:auto;background:#070709;border:1px solid var(--line);border-radius:15px;corner-shape:squircle;padding:14px;font:12px/1.62 ui-monospace,SFMono-Regular,Menlo,monospace}.ow-msg-body code{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;background:var(--surface);border-radius:6px;padding:2px 5px}.ow-msg-body pre code{padding:0;background:none}.ow-reason{color:var(--muted);font-size:12px;border-left:2px solid var(--line);padding:5px 0 5px 10px;margin-bottom:11px;white-space:pre-wrap}.ow-error{color:var(--bad)}
.ow-user{display:flex;justify-content:flex-end;padding-left:17%}.ow-user-bubble{max-width:min(80%,690px);border-radius:26px 26px 8px 26px;corner-shape:squircle;background:var(--surface);padding:10px 14px;font-size:14px;line-height:1.62;word-break:break-word}
.ow-composer-zone{position:relative;z-index:15;padding:8px 18px max(18px,env(safe-area-inset-bottom));background:linear-gradient(to bottom,transparent 0,var(--bg) 26px,var(--bg) 100%)}.ow-composer{width:min(100%,52rem);margin:0 auto;border:1px solid var(--line);border-radius:32px;corner-shape:squircle;background:var(--sidebar);box-shadow:0 9px 34px rgba(0,0,0,.18);padding:10px 10px 9px;transition:.14s var(--ease)}.ow-composer:focus-within{border-color:color-mix(in srgb,var(--fg) 23%,var(--line));box-shadow:0 10px 40px rgba(0,0,0,.22),0 0 0 3px color-mix(in srgb,var(--fg) 4%,transparent)}.ow-composer textarea{display:block;width:100%;height:46px;min-height:46px;max-height:180px;resize:none;overflow-y:auto;border:0;outline:0;background:transparent;color:var(--fg);padding:7px 7px 5px;line-height:1.55}.ow-compose-foot{height:36px;display:flex;align-items:center;gap:7px;color:var(--dim);font-size:10px;padding-left:5px}.ow-send{margin-left:auto;width:34px;height:34px;border:0;border-radius:12px;background:var(--fg);color:var(--bg);display:grid;place-items:center;cursor:pointer}.ow-send.stop{background:var(--bad);color:white}
...
@keyframes owpop{from{opacity:0;transform:translateY(-5px) scale(.985)}to{opacity:1;transform:none}}@keyframes owmsg{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:none}}@keyframes owslide{from{opacity:0;transform:translateX(9px)}to{opacity:1;transform:none}}
...
function renderConversation(){const c=current(),root=$('conversation');root.innerHTML='';if(!c.messages.length){root.innerHTML='<div class="ow-empty"><div class="ow-empty-inner"><div class="ow-empty-logo">B</div><h1>How can I help?</h1><p>Choose a model and start a conversation.</p><div class="ow-suggestions"><button class="ow-suggestion" onclick="usePrompt(\'Review this code and identify reliability risks\')">Review code and reliability</button><button class="ow-suggestion" onclick="usePrompt(\'Help me diagnose a failed API request\')">Diagnose an API request</button></div></div></div>';return}c.messages.forEach(m=>appendRendered(m.role,m.content,m.reasoning||''));requestAnimationFrame(()=>scrollBottom(false,true))}
function appendRendered(role,content,reasoning=''){const root=$('conversation'),empty=root.querySelector('.ow-empty');if(empty)empty.remove();const wrap=document.createElement('div');wrap.className='ow-message '+(role==='user'?'ow-user':'ow-assistant');if(role==='user'){wrap.innerHTML='<div class="ow-user-bubble">'+md(content)+'</div>';root.appendChild(wrap);return wrap.querySelector('.ow-user-bubble')}wrap.innerHTML='<div class="ow-avatar">B</div><div><div class="ow-msg-head">Bridgena</div><div class="ow-msg-body">'+(reasoning?'<div class="ow-reason">'+esc(reasoning)+'</div>':'')+md(content)+'</div></div>';root.appendChild(wrap);return wrap.querySelector('.ow-msg-body')}
...
async def _lifespan(app):
    ...
    log("INFO", "Control UI · v3.9.2 full UI rebuild · settings bodies + stronger squircles + fixed legacy component layouts")
